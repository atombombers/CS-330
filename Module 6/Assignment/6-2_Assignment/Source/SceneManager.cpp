...
void SceneManager::DefineObjectMaterials()
{
	...
	// Matte-ish material for most objects (good diffuse shading)

	OBJECT_MATERIAL matte;
	matte.tag = "matte";
	matte.ambientStrength = 0.08f;
	matte.ambientColor = glm::vec3(1.0f, 1.0f, 1.0f);
	matte.diffuseColor = glm::vec3(1.0f, 1.0f, 1.0f);
	matte.specularColor = glm::vec3(0.20f, 0.20f, 0.20f);
	matte.shininess = 16.0f;
	m_objectMaterials.push_back(matte);

	// Shinier material for the sphere so it has a visible highlight
	OBJECT_MATERIAL shiny;
	shiny.tag = "shiny";
	shiny.ambientStrength = 0.06f;
	shiny.ambientColor = glm::vec3(1.0f, 1.0f, 1.0f);
	shiny.diffuseColor = glm::vec3(1.0f, 1.0f, 1.0f);
	shiny.specularColor = glm::vec3(0.85f, 0.85f, 0.85f);
	shiny.shininess = 64.0f;
...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("lightSources[0].position", 15.0f, 2.5f, 1.0f);
	m_pShaderManager->setVec3Value("lightSources[0].ambientColor", 0.45f, 0.1f, 0.1f);
	m_pShaderManager->setVec3Value("lightSources[0].diffuseColor", 0.05f, 0.10f, 0.90f);
	m_pShaderManager->setVec3Value("lightSources[0].specularColor", 0.10f, 0.20f, 1.00f);
	m_pShaderManager->setFloatValue("lightSources[0].focalStrength", 32.0f);
	m_pShaderManager->setFloatValue("lightSources[0].specularIntensity", 0.35f);
	
	m_pShaderManager->setVec3Value("lightSources[1].position", -8.0f, 0.5f, -3.0f);
